...
BLOB_CONTAINER_NAME = "parking-raw-data" # 원본 데이터를 저장할 컨테이너 (새 이름 추천)
OUTPUT_FILENAME = "parking_data.csv" # 저장될 CSV 파일 이름 (고정)

def main():
    # --- 2. '어제' 날짜의 시작과 끝 타임스탬프 계산 ---
    # cron 작업의 안정성을 위해 UTC 기준으로 '어제'를 명확히 정의
    today_utc = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
    yesterday_utc = today_utc - timedelta(days=1)
    end_of_yesterday_utc = today_utc - timedelta(microseconds=1)

    # Cosmos DB의 _ts는 Unix 타임스탬프 (초 단위)
    start_timestamp = int(yesterday_utc.timestamp())
    end_timestamp = int(end_of_yesterday_utc.timestamp())
    
    processing_date_str = yesterday_utc.strftime('%Y-%m-%d')
    print(f"스크립트 실행 시작: '{processing_date_str}' 날짜의 데이터를 처리합니다.")
    print(f"조회 시간 범위 (UTC): {yesterday_utc.isoformat()} 부터 {end_of_yesterday_utc.isoformat()} 까지")
    print(f"타임스탬프 범위: {start_timestamp} 부터 {end_timestamp} 까지")

    # --- 3. Cosmos DB에서 '어제' 데이터 가져오기 ---
    client = CosmosClient(COSMOS_ENDPOINT, credential=COSMOS_KEY)
    database = client.get_database_client(DATABASE_NAME)
    container = database.get_container_client(CONTAINER_NAME)

    # _ts 필터를 사용하여 지정된 시간 범위의 모든 문서를 가져오는 쿼리
    ...
    # 'yyyy/MM/dd' 형식의 폴더 경로 생성
    blob_path = f"{yesterday_utc.strftime('%Y/%m/%d')}/{OUTPUT_FILENAME}"
    ...
    blob_client.upload_blob(output_csv, overwrite=True) # 재실행 가능하도록 overwrite=True 유지
    print("업로드 성공!")
